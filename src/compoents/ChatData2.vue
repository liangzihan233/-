<template>
    <div class="chart-container">
        ...
        <div class="desc">
            ...
            <div class="left">
                <div><span class="nickname">{{chartData.mynickname}}</span>发送了<span class="time-period">{{
                    chartData.mynum }}</span>条消息</div>
                <div><span class="nickname">{{chartData.hisnickname}}</span>发送了<span class="time-period">{{
                    chartData.tanum }}</span>条消息</div>
            </div>
            <div class="right">
                <div>谁是话痨呢？</div>
            ...
            <div class="dog-image">
                <img src="../../public/dog/7.png" alt="Dog" />
            </div>
        </div>
    </div>
</template>
...
import { chatdescription,chartData } from '../js/getdata.js';
...
const drawChart = (chartId, index, timePeriodData) => {
    ...
    const indicator = ['文本','图片','视频','表情包','其他'];
    // 将 my_type_count 和 ta_type_count 转换为对象
    const myCountMap = Object.fromEntries(chartData.value.my_type_count);
    const taCountMap = Object.fromEntries(chartData.value.ta_type_count);
    // 创建数值数组
    ...
    const option = {
        ...
        legend: {
            data: [chartData.value.mynickname, chartData.value.hisnickname],
            top:'0px',
            left:0,
            textStyle: {
                color: '#ffffff', // 设置文字颜色
        ...
        series: [
        {
            ...
            data: [
                {
                    value: myValues ,
                    name: chartData.value.mynickname,
                    symbol: 'rect',
                    symbolSize: 10,
                    lineStyle: {
                        type: 'dashed'
                ...
                {
                    value: taValues,
                    name: chartData.value.hisnickname,
                    label: {
                        show: true,
                        textStyle: {
                            color: '#ffffff', // 设置文字颜色为红色
