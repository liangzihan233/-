...
var __vue_render__ = function (component) {
  var _vm = component;

  var stackChildren = _vm.stack.map(function (card, index) {
    return h('div', {
      key: card._id,
  ...
  var navSlot = _vm.$slots.nav ? _vm.$slots.nav({
    activeCardIndex: _vm.originalActiveCardIndex,
    onNext: _vm.onNext,
    onPrevious: _vm.onPrevious
  }) : null;

  var children = [stackDiv, navSlot];
...
__vue_component__.styles = `
.vue-card-stack__wrapper{position:relative}
.vue-card-stack__stack{position:relative;overflow:hidden}
.vue-card-stack__card{position:absolute;transform-origin:0 50%;cursor:grab}
`;
